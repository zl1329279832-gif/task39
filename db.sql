...
CREATE TABLE IF NOT EXISTS drill_checkpoint (
    ...
    vuln_category VARCHAR(50) NOT NULL COMMENT '漏洞分类',
    checkpoint_order INT NOT NULL DEFAULT 0 COMMENT '检查点顺序',
    mode VARCHAR(20) NOT NULL COMMENT 'EXPLOIT or DEFENSE',
    vuln_endpoint VARCHAR(500) NOT NULL COMMENT '漏洞端点',
    sec_endpoint VARCHAR(500) NOT NULL COMMENT '安全端点',
    http_method VARCHAR(10) NOT NULL DEFAULT 'GET',
    target_param VARCHAR(100) COMMENT '目标参数',
    max_score INT NOT NULL DEFAULT 100 COMMENT '最高分数',
    max_hints INT NOT NULL DEFAULT 3 COMMENT '最大提示数',
    time_limit INT DEFAULT 1800 COMMENT '时间限制（秒）',
    prerequisite_id INT DEFAULT NULL COMMENT '前置检查点ID',
    verify_pattern VARCHAR(1000) COMMENT '验证正则',
    defense_pattern VARCHAR(1000) COMMENT '防御验证正则',
    hint_content TEXT COMMENT '提示内容',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_checkpoint_task FOREIGN KEY (task_id) REFERENCES drill_task(id)
...
CREATE TABLE IF NOT EXISTS drill_attempt (
    ...
    payload_summary TEXT COMMENT '载荷摘要',
    evidence TEXT COMMENT '成功证据',
    elapsed_seconds INT DEFAULT 0,
    hints_used INT DEFAULT 0,
    deduction_items VARCHAR(500) COMMENT '扣分项JSON',
    score INT DEFAULT 0,
    passed TINYINT DEFAULT 0,
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uk_user_checkpoint (user_id, checkpoint_id),
...
CREATE TABLE IF NOT EXISTS drill_score_summary (
    ...
    CONSTRAINT fk_summary_task FOREIGN KEY (task_id) REFERENCES drill_task(id),
    CONSTRAINT fk_summary_user FOREIGN KEY (user_id) REFERENCES Admin(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='演练成绩汇总表';